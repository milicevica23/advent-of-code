...
fn parse_game_part_2(input: &str) -> Vec<Game> {
    let rows = input.split("\n").collect::<Vec<_>>();

    rows.iter().map(
        |each_row| match *each_row {
            "A X" => Game {oponent_move: Move::Rock, my_move: Move::Scissors},
            "A Y" => Game {oponent_move: Move::Rock, my_move: Move::Rock},
            "A Z" => Game {oponent_move: Move::Rock, my_move: Move::Paper},
            ...
            "C X" => Game {oponent_move: Move::Scissors, my_move: Move::Paper},
            "C Y" => Game {oponent_move: Move::Scissors, my_move: Move::Scissors},
            "C Z" => Game {oponent_move: Move::Scissors, my_move: Move::Rock},
...
mod test {
    ...
    fn play_game(){
        ...
        let games = parse_game_part_2(&contents);
